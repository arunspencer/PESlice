...
from peslice.utils.graphs import define_graphs, get_nearest_neighbour_distance

class LinearSlicePES(BaseSlicePES):
    """
    Slice a potential energy surface (PES) along a n-dimensional hyperplane (n = 1, 2, 3)

    Hyperplanes are defined by a point and ``n-1`` basis vectors
    
    Vectors contain orthogonal sub-vectors of length 3 - corresponding to atomic Cartesian coordinates

    PES slices can be evaluated using a parsed ``graph-pes`` NN model

    The hyperplane is defined by the equation: ``y = m0 + c1*m1 + c2*m2 + c3*m3 + ...``
    ...
    get_nn_distances
        Whether to calculate the smallest nearest neighbour distances for each sample point along the hyperplane
    ...
    point_density
        The number of points to sample along each dimension in the hyperplane per unit length
    ranges_n
        The value of n to define the range ``[-n, n]`` to sample the plane coefficients
    slice_dim
    ...
    def def_slice(
        ...
        *args
            ...
            Shape ``(function_dim, )``
        model_cutoff
            The cutoff distance for the parsed ``graph-pes`` model
    ...
    def _generate_points(self) -> None:
        """
        Generates sample points in the hyperplane from the orthogonal basis vectors, ``ranges_n``, and ``point_density``
        """
    ...
    def _generate_atoms(self) -> list[ase.Atoms]:
        """
        Generates the ``ase.Atoms`` objects for each sample point along the hyperplane
        """
